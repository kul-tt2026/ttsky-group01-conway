...
_[VGA (video graphics array)](https://en.wikipedia.org/wiki/Video_Graphics_Array)_ is an old technology, which means that it is easy to interface with. There are many different graphics modes, but this project has choosen the most common: _[640x480@60Hz](http://microvga.com/vga-timing/640x480@60Hz)_. The timing diagram is shown below.
...
The `vga_get_cell_idx` module calculates the coordinates of the current cell in the grid, based of the position of the pixel on the screen. In addition to the cell's `col_idx`/`row_idx`, it also outputs `pixel_col_offset` and `pixel_row_offset`, which indicate the pixel position within that cell. This info is later used to draw the cursor icon correctly.
