                ...
                for row_idx in range(num_rows):
                    for cursor_col in range(num_cols):
                        for cursor_row in range(num_rows):
                            ...
                            dut.simulation_running.value = sim_running
                            ...
                            cursor_match = (cursor_col == col_idx) and (cursor_row == row_idx)
                            expect_cursor = (not sim_running) and cursor_match

                            expected = 0b10 if expect_cursor else ((0 << 1) | cell_mem)

                            actual = int(dut.cell_type.value)
                            assert actual == expected, (
                                f"Mismatch: sim_running={sim_running}, "
